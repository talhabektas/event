...
const UserContentGrid: React.FC<{ items: any[], renderItem: (item: any) => React.ReactNode, noContentMessage: string }> = ({ items, renderItem, noContentMessage }) => (
    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {items.length > 0 ? items.map((item, index) => <div key={item.id || index}>{renderItem(item)}</div>) : <div>{noContentMessage}</div>}
    </div>
);
...
const UserEvents: React.FC<{ events: EventItem[] }> = ({ events }) => <UserContentGrid items={events} noContentMessage="Gösterilecek etkinlik yok." renderItem={event => (
    <Card>
        ...
        <div className="mt-2 text-sm text-muted-foreground flex items-center"><MapPinIcon className="h-4 w-4 mr-2 shrink-0" /><span>{event.location}</span></div>
        <div className="mt-1 text-sm text-muted-foreground flex items-center"><CalendarIcon className="h-4 w-4 mr-2 shrink-0" /><span>{formatDate(event.final_start_time)}</span></div>
...
const UserRooms: React.FC<{ rooms: Room[] }> = ({ rooms }) => <UserContentGrid items={rooms} noContentMessage="Kullanıcı herhangi bir odaya üye değil." renderItem={room => (
    <Card>
        ...
        <div className="mt-2 text-sm text-muted-foreground flex items-center"><UserGroupIcon className="h-4 w-4 mr-2 shrink-0" /><span>{room.memberCount} üye</span></div>
